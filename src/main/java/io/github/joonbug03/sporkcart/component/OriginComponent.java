...
public record OriginComponent(BlockPos pos) implements TooltipAppender {
    public static final Text FIRST_SELECTION = Text.translatable("item.sporkcart.track.origin").formatted(Formatting.YELLOW);
    public static final Text HOW_TO_CLEAR = Text.translatable("item.sporkcart.track.clear_hint").formatted(Formatting.GOLD, Formatting.ITALIC);
