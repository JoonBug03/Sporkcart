        ...
        if (passenger != null) {
            if (!hadPassenger) {
                ...
                if (this.splinePieceProgress > 1) {
                    if( (endE.getCachedState().isOf(Sporkcart.SHUTTLE_TIES) || endE.getCachedState().isOf(Sporkcart.INVISIBLE_SHUTTLE_TIES) ) && endE.getCachedState().get(ShuttleTiesBlock.RUNNING)) {
                        ...
                        if (nextE == null) {
                            ...
                            if (endE.getCachedState().isOf(Sporkcart.SWITCH_TIES) || endE.getCachedState().isOf(Sporkcart.INVISIBLE_SWITCH_TIES)) {
                    ...
                    if(startE.getCachedState().isOf(Sporkcart.SHUTTLE_TIES) && startE.getCachedState().get(ShuttleTiesBlock.RUNNING)) {
                        this.reversed = !reversed;
                        return;
                    } else {
                        this.splinePieceProgress += 1;
                ...
                if (startE.nextType() == TrackType.STATION || startE.nextType() == TrackType.INVISIBLE_STATION) {
                    if(power > 0) {
                        double speed = power*0.01;
                        this.trackVelocity = Math.max(this.trackVelocity, speed);
                    } else {
